...
test('desktop visitor can enter, walk, offer flowers, and collect prasad', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('#flower-status')).toHaveText('✓ फुले अर्पण केली');
  await expect(page.locator('#toast')).toBeHidden({ timeout: 6000 });
  ...
  await expect(page.locator('#dialog-title')).toHaveText('आपल्यासाठी बाप्पांचा गोड प्रसाद');
  await page.locator('#close-dialog').click();
  await expect(page.locator('#interaction-button')).toBeDisabled();
  await expect(page.locator('#prasad-status')).toHaveText('✓ प्रसाद मिळाला');
...
test('mobile has no horizontal overflow and supports touch navigation', async ({ browser }, testInfo) => {
  ...
  await page.keyboard.down('w');
  await expect(page.locator('#interaction-button')).toContainText('फुले अर्पण करा', { timeout: 15000 });
  await page.keyboard.up('w');
  await page.locator('#interaction-button').tap();
  await expect(page.locator('#flower-status')).toHaveText('✓ फुले अर्पण केली');
  await expect(page.locator('#toast')).toBeHidden({ timeout: 6000 });
